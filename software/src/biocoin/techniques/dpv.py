"""
Differential pulse voltammetry technique implementation.
"""
...
class DPVConfig:
    """
    Configuration payload fields for differential pulse voltammetry.
    """
...
class DifferentialPulseVoltammetry(BaseTechnique[float]):
    """
    Implements differential pulse voltammetry (DPV) on the Biocoin device.

    DPV applies a staircase baseline (Estart -> Estop in Estep increments).
    ...
    async def configure(
        ...
        Parameters:
            ...
            - max_current (float): Maximum expected current (uA), (0, 3000]
        ...
        Returns:
            - None
        """
        logger.info('Sending DPV technique parameters...')
        ...
        payload = _config.to_payload(self.Technique.DPV)
    ...
    async def run(self) -> np.ndarray:
        ...
        """
        if self.V is None:
            raise RuntimeError('DPV must be configured before run()')

        results = await self._run_streaming_until_done(self.duration)

        logger.info(f'Received {len(results)} data points from DPV.')

        diffs = pairwise_differential_current(results, second_minus_first=False, label='DPV')
        if len(diffs) != len(self.V):
            raise RuntimeError(f'Expected {len(self.V)} DPV points, received {len(diffs)}')
    ...
    async def notification_handler(self, _: int, data: bytes) -> None:
        ...
        Returns:
            - None
        """
        self._ingest_float32_samples(data, label='DPV')
