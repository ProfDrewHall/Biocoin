"""
Standalone TEMP example CLI for running temperature sensing with explicit parameters.
"""
...
from biocoin.techniques import Temperature
...
def write_csv(data: np.ndarray, output_path: Path) -> None:
    """
    Write TEMP output to CSV.

    Parameters:
        - data (np.ndarray): 2D array with [time (s), temperature (degC)] columns.
...
def build_parser() -> argparse.ArgumentParser:
    """
    Build parser for standalone TEMP example runner.
    ...
    """
    parser = argparse.ArgumentParser(description='Run TEMP with explicit per-technique parameters.')
    parser.add_argument('--device-name', default='Biocoin', help='BLE device name to connect to.')
    parser.add_argument('--output', type=Path, default=Path('./results/TEMP_output.csv'), help='Output CSV path.')
    ...
    parser.add_argument('--channel', type=int, default=DEFAULT_CHANNEL, help='Electrode channel index (0-2).')
    return parser


async def run(args: argparse.Namespace) -> None:
    """
    Execute TEMP technique with provided CLI arguments.
    ...
    logger.info('Starting standalone TEMP run.')
    ...
    try:
        ...
        data = await temp.run(duration=args.duration)
        write_csv(data, args.output)
        logger.info(f'Saved {len(data)} TEMP points to {args.output}')
    except BiocoinError:
        raise
...
def main() -> None:
    """
    Parse CLI arguments and execute the async TEMP run.
    ...
    try:
        ...
        logger.exception('TEMP run failed.')
