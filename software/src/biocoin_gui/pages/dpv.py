...
PAGE = PAGE_METADATA['dpv']
CURRENT_LABEL = 'Current (µA)'
DEFAULT_MAX_CURRENT_UA = 100.0
...
initialize_experiment_state('dpv')
...
async def run_dpv_experiment(
    ...
    *,
    ...
    controller.bind_abort(asyncio.get_running_loop(), lambda: device.write_ctrl_command(dpv.Command.STOP))
...
def finalize_dpv_run() -> BackgroundTechniqueRun | None:
    return finalize_experiment_run(
        'dpv',
        aborted_message='Differential Pulse Voltammetry experiment aborted.',
    )
...
def start_dpv_run(
    ...
    *,
    ...
    st.session_state.dpv_pending_experiment = {
        'name': exp_name,
        'meta': {
    ...
    st.session_state.dpv_message = None
    st.session_state.dpv_run = start_background_run(
        lambda controller: run_dpv_experiment(
            controller,
            device,
...
dpv_run = finalize_dpv_run()
dpv_snapshot = dpv_run.snapshot() if dpv_run is not None else None
...
with cols[0].container(border=True):
    ...
    max_current = st.number_input(
        ...
        min_value=1.0,
        max_value=3000.0,
        value=DEFAULT_MAX_CURRENT_UA,
        step=50.0,
    )
    channel = st.selectbox('Channel', options=[0, 1, 2, 3])

    processing_interval_min = max(pulse_width / 1000.0, 0.01)
    ...
    exp_name_input = st.text_input(
        ...
        disabled=dpv_snapshot is not None and dpv_snapshot.is_running,
    )

    action_label = 'Abort Experiment' if dpv_snapshot is not None and dpv_snapshot.is_running else 'Run Experiment'
    action_clicked = st.button(action_label, width='stretch', disabled=not st.session_state.connected)

    if action_clicked:
        if dpv_snapshot is not None and dpv_snapshot.is_running:
            dpv_run.request_abort()
        else:
...
with cols[1].container(border=True):
    ...
    if st.session_state.dpv_message is not None:
        level, message = st.session_state.dpv_message
        getattr(st, level)(message)

    if dpv_snapshot is not None and dpv_snapshot.is_running:
        if dpv_snapshot.abort_requested:
            st.warning(dpv_snapshot.status_text)
        else:
            st.info(dpv_snapshot.status_text)
        st.progress(dpv_snapshot.progress, text=dpv_snapshot.status_text)

        live_df = dpv_snapshot.live_data
        if isinstance(live_df, pd.DataFrame) and not live_df.empty:
            fig = px.line(live_df, x='Potential (mV)', y=CURRENT_LABEL, title='Live DPV Data')
            st.plotly_chart(fig, width='stretch', key='dpv_live_plot')
    ...
    if st.session_state.dpv_experiments:
        ...
        if display_mode == 'Plot':
            fig = px.line(title='DPV Results', labels={'x': 'Potential (mV)', 'y': CURRENT_LABEL})
            for name in selected_exps:
                df = st.session_state.dpv_experiments[name].data
                fig.add_scatter(x=df['Potential (mV)'], y=df[CURRENT_LABEL], name=name)
            st.plotly_chart(fig, width='stretch', key='dpv_results_plot')
        else:
            combined_df = pd.concat(
                [
                    record.data.assign(Experiment=name)
                    for name, record in st.session_state.dpv_experiments.items()
        ...
        st.divider()
        render_notes_editor(st.session_state.dpv_experiments, key_prefix='dpv')
        ...
        render_experiment_downloads(
            ...
            selected_exps,
            csv_filename='DPV_results.csv',
            excel_filename='DPV_results.xlsx',
            key_prefix='dpv',
        )
