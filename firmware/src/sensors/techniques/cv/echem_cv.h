...
namespace sensor {
  constexpr uint8_t kCVMaxChannel = 3u;
  constexpr uint32_t kCVMinFifoThreshold = 1u;
  constexpr uint32_t kCVMaxFifoThreshold = 1023u;
  constexpr uint32_t kCVWakeSleepTicks = 4u;
  constexpr uint32_t kCVWakeAdjustTicks = 6u;
  constexpr uint32_t kCVSeqRefSettleUs = 250u;
  constexpr uint32_t kCVSeqPostAdcDelayClks = 20u;

  struct CVParameterPayload {
  ...
  static_assert(sizeof(CVParameterPayload) == 29, "CVParameterPayload must remain packed at 29 bytes");
  ...
  struct CVConfig {
    ...
    float RampStepMv;    /**< Signed ramp step used by DAC sequence generation [mV]. */
  ...
  private:
    /** @brief Initialize CV configuration defaults. */
    ...
    bool validateParameters(const CVParameterPayload& params, String* reason) const;
