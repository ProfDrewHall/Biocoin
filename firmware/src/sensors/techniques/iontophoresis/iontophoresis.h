...
namespace sensor {
  struct IontophoresisParameterPayload {
    float samplingInterval;
    float stimCurrent;
    float maxCurrent;
  } __attribute__((packed));
  static_assert(sizeof(IontophoresisParameterPayload) == 12,
                "IontophoresisParameterPayload must remain packed at 12 bytes");

  struct IontophoresisConfig {
  ...
  public:
    ...
    // Data retrieval (no queued sample stream for this technique)
  ...
  private:
    /** @brief Initialize iontophoresis configuration defaults. */
    ...
    bool validateParameters(const IontophoresisParameterPayload& params, String* reason) const;
    void cleanupStartFailure(const String& reason) const;

    TaskHandle_t stimulationTaskHandle;

    IontophoresisConfig config;
