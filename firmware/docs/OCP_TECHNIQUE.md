# Open Circuit Potential (OCP) Notes

## Host Parameter Constraints

- Payload layout (`OCPParameterPayload`) is fixed-size packed data (`9` bytes).
...
- `channel` must be one of the allowed OCP mux channels.
...
- On `ENDSEQ`, OCP stops and publishes `NOT_RUNNING` (or `ERROR` if stop fails).
